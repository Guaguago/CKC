...
class CoGraphMatcher(nn.Module):
    def __init__(self, embed_size, vocab_size, gnn_hidden_size, gnn_layers, encoder_hidden_size, encoder_layers,
                 n_heads, gnn, encoder, aggregation, use_keywords, keyword_encoder, keyword_score_weight=1, dropout=0,
                 CN_hopk_edge_matrix_mask=None, nodeid2wordid=None, keywordid2wordid=None, keywordid2nodeid=None,
                 concept_encoder="mean", combine_word_concepts="concat"):
        ...
        if self.encoder == "GRU":
            self.utterance_encoder = nn.GRU(encoder_input_size, encoder_hidden_size, encoder_layers, batch_first=True,
                                            dropout=dropout, bidirectional=True)
            self.candidate_encoder = nn.GRU(encoder_input_size, encoder_hidden_size, encoder_layers, batch_first=True,
                                            dropout=dropout, bidirectional=True)
    ...
    def score(self, x, y):
        ...
        return torch.bmm(x.reshape(batch_size * num_candidates, 1, emb_size),
                         y.reshape(batch_size * num_candidates, emb_size, 1)).reshape(batch_size,
    ...
    def encode_context_offline(self, context, context_concepts, conetxt_keywords, CN_hopk_edge_index):
        ...
        if CN_hopk_edge_index is not None:
            ...
            if conetxt_keywords is not None:
                context_keywords_concept_out = self.encode_keywords(self.CN_hopk_out, self.keywordid2nodeid[
                    conetxt_keywords])  # (batch, emb_size)
        ...
        if conetxt_keywords is not None:
            # encode keywords
            context_keywords_out = self.encode_keywords(self.embedding.weight,
                                                        self.keywordid2wordid[conetxt_keywords])  # (batch, emb_size)

        context_out, context_mask = self.encode_context(self.embedding.weight,
                                                        context)  # (batch, m, emb), where m can be context_len*seq_len or context_len
    ...
    def encode_candidate_offline(self, candidate, candidate_concepts, candidate_keywords, CN_hopk_edge_index):
        ...
        if CN_hopk_edge_index is not None:
            ...
            if candidate_keywords is not None:
                candidate_keywords_concept_out = self.encode_keywords(self.CN_hopk_out, \
                                                                      self.keywordid2nodeid[candidate_keywords].reshape(
                                                                          batch_size * num_candidates,
                                                                          -1))  # (batch*num_candidates, emb_size)
        ...
        if candidate_keywords is not None:
            ...
            candidate_keywords_out = self.encode_keywords(self.embedding.weight, \
                                                          self.keywordid2wordid[candidate_keywords].reshape(
                                                              batch_size * num_candidates,
                                                              -1))  # (batch*num_candidates, emb_size)

        # encoding
        candidate_out, candidate_mask = self.encode_candidate(self.embedding.weight,
                                                              candidate)  # (batch*num_candidates, n, emb)
    ...
    def predict(self, context_out, context_mask, context_concept_out, context_concept_mask,
                context_keywords_concept_out, context_keywords_out, \
                candidate_out, candidate_mask, candidate_concept_out, candidate_concept_mask,
                candidate_keywords_concept_out, candidate_keywords_out):
        """
        ...
        out = torch.bmm(context_out.unsqueeze(1),
                        candidate_out.reshape(batch_size, num_candidates, -1).transpose(1, 2)).squeeze(
            1)  # (batch, num_candidates)
        out = out + self.keyword_score_weight * keywords_score
        return out  # (batch, num_candidates)

    def forward(self, context, candidate, conetxt_keywords=None, candidate_keywords=None, context_concepts=None,
                candidate_concepts=None, \
        ...
        if CN_hopk_edge_index is not None:
            ...
            # encode keywords
            if conetxt_keywords is not None:
                context_keywords_out = self.encode_keywords(CN_hopk_out, self.keywordid2nodeid[
                    conetxt_keywords])  # (batch, emb_size)
                if candidate_keywords is not None:
                    ...
                    candidate_keywords_out = self.encode_keywords(CN_hopk_out, \
                                                                  candidate_concepts.reshape(
                                                                      batch_size * num_candidates,
                                                                      -1))  # (batch*num_candidates, emb_size)
                keywords_concept_score = torch.bmm(candidate_keywords_out.reshape(batch_size, num_candidates, -1), \
                                                   context_keywords_out.unsqueeze(-1)).squeeze(
                    -1)  # (batch, num_candidates)
            ...
            # concept matching
            context_concept_out = context_concept_out.repeat_interleave(num_candidates,
                                                                        dim=0)  # (batch*num_candidates, m, emb)
            context_concept_mask = context_concept_mask.repeat_interleave(num_candidates,
                                                                          dim=0)  # (batch*num_candidates, m, emb)

            # aggregation
            context_concept_out = self.aggregate(context_concept_out,
                                                 context_concept_mask)  # (batch*num_candidates, emb)
            candidate_concept_out = self.aggregate(candidate_concept_out,
                                                   candidate_concept_mask)  # (batch*num_candidates, emb)
        ...
        if conetxt_keywords is not None:
            ...
            if candidate_keywords is not None:
                ...
                candidate_keywords_out = self.encode_keywords(self.embedding.weight, \
                                                              candidate.reshape(batch_size * num_candidates,
                                                                                -1))  # (batch*num_candidates, emb_size)
            keywords_score = torch.bmm(candidate_keywords_out.reshape(batch_size, num_candidates, -1), \
                                       context_keywords_out.unsqueeze(-1)).squeeze(-1)  # (batch, num_candidates)
            if CN_hopk_edge_index is not None:
        ...
        # scoring
        out = self.score(context_out.reshape(batch_size, num_candidates, -1),
                         candidate_out.reshape(batch_size, num_candidates, -1))  # (batch, num_candidates)
        if conetxt_keywords is not None:
            out = out + self.keyword_score_weight * keywords_score
        return out
